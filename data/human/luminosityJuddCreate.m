...
vLambda = vcReadSpectra('luminosity',wave);
vcNewGraphWin;
plot(wave,vLambda);
...
lumJudd = vcReadSpectra('luminosityJudd',wavelength);

smithPokornyCones = vcReadSpectra('smithPokornyCones',wavelength);
tmp = smithPokornyCones*[1,.5,0]';
tmp = tmp/max(tmp(:));

lum = vcReadSpectra('luminosity',wavelength);
lum = lum/max(lum(:));
...
stockman = vcReadSpectra('stockman.mat',wavelength);
tmp = stockman*[1,.5,0]';
tmp = tmp/max(tmp(:));
