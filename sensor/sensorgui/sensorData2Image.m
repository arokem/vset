function img = sensorData2Image(ISA,dataType,gam,scaleMax)
% Produce the image data displayed in the sensor window.
%
%   img = sensorData2Image(ISA,[dataType = 'volts'],[gam=1],[scaleMax=0 (false)])
%
...
nSensors   = sensorGet(ISA,'nFilters');
expTimes   = sensorGet(ISA,'expTimes');
nExposures = sensorGet(ISA,'nExposures');

if nExposures > 1

    pSize = size(sensorGet(ISA,'pattern'));
    if isequal(pSize,size(expTimes))
        ...
        expPlane = sensorGet(ISA,'exposurePlane');
        img = sensorGet(ISA,dataType);
        img = img(:,:,expPlane);
    end
end
...
%   Old code ... probably never needed in the first place
%
% if size(img,3) > 3 || size(img,3) == 2
%     Rarely invoked, but it might be if we have multiple sensors.  In that
%     case, I am not sure it makes sense.
%     [row,col,nWave] = size(img); %#ok<NASGU>
%     img = replaceNaN(img,0)/mxImage;
%     img = RGB2XWFormat(img);
%     RGB = imageSPD2RGB(img,w,gam);
%     img = XW2RGBFormat(RGB,row,col);
%
% elseif  size(img,3) == 3
%     Conventional RGB display.  We scale the display intensity relative to the
%     voltage swing of the pixel.
%     - This condition doesn't make any sense to me.  Not sure why it's
%     here.
%     img = img/mxImage;
%     img = replaceNaN(img,0);
%     img = ieClip(img,0,[]).^gam;
%
%-----------------------------------------------------
function T = sensorDisplayTransform(ISA)
%
%  T = sensorDisplayTransform(ISA)
%
%   Find a transform that maps the ISA filter spectra into a an
...
bMatrix = colorBlockMatrix(sensorGet(ISA,'wave'),0.2);
