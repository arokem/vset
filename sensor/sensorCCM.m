...
%
% We plan to write various ways of defining a color conversion matrix (CCM)
% from sensor data to display data. At present, only the Macbeth method is
% implemented.  
%
% ccmMethod = 'macbeth'
...
% about the locations of particular targets in the sensor data.  Normally,
% this information is obtained by a user selection, and the spatial 
% information is returned in pointLoc.  If you already know the pointLoc
% values, you can send them in and skip that interactive part of 
%
% If there is no return argument, evaluation graphs are displayed.
%
% See also: s_macbethDeltaE, macbethEvaluationGraphs, sensorCCM
%
% Examples:
%   User selects corners; the routine m akes graphs and prints the linear
...
switch lower(ccmMethod)
    case 'macbeth'
        ...
        % idealRGB = rgb*L, so L = pinv(rgb)*idealRGB
        L = pinv(rgb)*idealRGB;
