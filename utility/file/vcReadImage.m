...
switch lower(imageType)
    ...
    case {'multispectral','hyperspectral'}
        ...
        disp('Reading multispectral data.')
        ...
        if ieVarInFile(variables,'mcCOEF')
            ...
            if ~isempty(varargin) && ~isempty(varargin{1})
                wavelength = basis.wave;
                newWave    = varargin{1};
                nBases     = size(basis.basis,2);
                extrapVal  = 0;
                newBases   = zeros(length(newWave),nBases);
                for ii=1:nBases
                    newBases(:,ii) = interp1(wavelength(:), basis.basis(:,ii), newWave(:),'linear',extrapVal);
                end
            ...
            if ieVarInFile(variables,'imgMean')
                ...
                if ~isempty(varargin)&& ~isempty(varargin{1})
                    ...
                    newWave    = varargin{1};
                    extrapVal  = 0;
                    imgMean = interp1(wavelength(:), imgMean(:), newWave(:),'linear',extrapVal);
                end
            ...
            % The photons are stored, no linear model.  We use the
            % basis slots.
            load(fullname,'photons','comment','wave');
            % Use this slot because sceneFromFile is expecting something
            mcCOEF = [];
            if ~isempty(varargin) && ~isempty(varargin{1})
                newWave = varargin{1};
                perfect = 0;
        ...
        if ~isempty(varargin)&& ~isempty(varargin{1})
            wavelength = illuminant.wavelength;
            newWave    = varargin{1};
            extrapVal  = 0;
            illuminant.data = ...
                interp1(wavelength(:), illuminant.data(:), newWave(:),'linear',extrapVal);
            illuminant.wavelength = newWave;
        end
...
% Check whether a particular name is in the variables file list.
% function bool = varInFile(variables,varName)
% %
% bool = 0;
