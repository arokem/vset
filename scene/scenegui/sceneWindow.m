...
% Should be replaced by plotScene call
scenePlotLuminance(scene,'linear');
...
% Edit | Adjust SPD | Set Illuminant

scene = vcGetObject('scene');

% Create the scale factor to transform from current to new illuminant
% If you read these in energy, you can skip the Energy2Quanta conversion
% below.  Duh.
...
if isempty(curIll)
    ...
    curIll = vcReadSpectra('d65',wave);
    scene  = sceneSet(scene,'illuminantPhotons',curIll);   
end

% Read the new file
fullName = vcSelectDataFile('stayput','r','mat','Select illuminant file');

mLum     = sceneGet(scene,'meanLuminance');
wave     = sceneGet(scene,'wave');

% Fix this to handle cancellation
T        = vcReadSpectra(fullName,wave);
illPhotons = Energy2Quanta(T,wave);
illFactor = illPhotons ./ curIll;

% Adjust the illuminant by the scale factor
skipIlluminant = 0;
