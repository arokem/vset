...
%        {'illuminantenergy'} - energy data
%        {'illuminantwave'}   - wavelength samples - deprecated to wave
...
switch parm
    ...
    case {'dangular','diagonalangular','diagonalfieldofview'}
        ...
        %  e = zeros(1,N);
        %  for ii=1:N
        %      scene = sceneSet(scene,'fov',ii);
        %      d(ii) = sceneGet(scene,'diagonal field of view');
        %      e(ii) = sqrt(sceneGet(scene,'wAngular')^2 + sceneGet(scene,'hAngular')^2);
        %  end
        %  vcNewGraphWin;  plot(e,d,'.'); grid on
   ...
   case {'photons','cphotons'}
    ...
    case {'reflectance'}
        ...
        % 
        % radiance = vcGetROIData(scene,vcRect2Locs(rect),'photons');
    ...
    case {'whitexyz','illuminantxyz'}
        % XYZ coordinates of illuminant, which is also the scene white point.
        energy = sceneGet(scene,'illuminant energy');
        wave   = sceneGet(scene,'wave');
        % Energy must be a row vector
        val    = ieXYZFromEnergy(energy(:)',wave);
        
    case {'lms','datalms','cone'}
        % sceneGet(scene,'lms');
        % 3D array of scene LMS values (Stockman)
        wave    = sceneGet(scene,'wave');
    ...
    case {'illuminantdata','illuminantenergy'}
        % The data field is supposed to have illuminant in standard energy
        % units.  But I have noticed that sometimes it doesn't.  Sigh.  Not
        % often, and I don't know when.
        % Force the return as a column vector.
        if checkfields(scene,'illuminant','data')
            val = scene.illuminant.data(:); 
        end
    case {'illuminantphotons'}
        val = sceneGet(scene,'illuminant energy');
        if isempty(val), return;
        else
            W = sceneGet(scene,'wave');
            val = Energy2Quanta(W,val(:));
        end
        val = val(:);
    case {'illuminantwave'}
    ...
    case {'rgb','rgbimage'}
        ...
        [row,col,nWave] = size(photons); %#ok<NASGU>
