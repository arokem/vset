...
for ii=1:length(wave)
    ...
    % Sometimes we have some annoying complex values left after this
    % filtering.  We get rid of it by an abs() operator.  It should never
    % be there.  But we think it arises because of rounding error.  
    % figure(1); imagesc(abs(filteredIMG)); colormap(gray)
    %
    % What about dmin and dmax on the 'cphotons' set.  Is that OK?
    oi = oiSet(oi,'cphotons',filteredIMG,wave(ii));
